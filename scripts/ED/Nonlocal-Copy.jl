...
filename = string("Cormat_Data_L=", L, "_Delta=", δ, ".txt")
mkdir(filename)

A = string(filename, "/Correlation_Matrix.txt")
B = string(filename, "/Eigenvalues.txt")
C = string(filename, "/Hvec.txt")
...
open(A, "w") do io
    DelimitedFiles.writedlm(A, sizes)
end
open(B, "w") do io
    DelimitedFiles.writedlm(B, Ts)
end
open(C, "w") do io
    DelimitedFiles.writedlm(B, Ts)
end
...
for k in 1:(L÷2)
    ...
    push!(fulldat, eigvals(transpose(cutoffmat(k, L)) * cm * cutoffmat(k, L)))
    ##need to fix this to move it back to position space
    push!(fullops, eigvecs(transpose(cutoffmat(k, L)) * cm * cutoffmat(k, L)))
